...
//#region Functions declaration init
  ...
  function loadNextQuestionComponent(questionIndex){
     ...
     const questionType = currentQuestion.value.questionType.codeName;
     switch (questionType) {
       case "qt-select-one":
  ...
  function getAnswersFromCurrentComponent(){
    ...
    selectedAnswers.value =  Array.from(toRaw(selectedAnswers.value).filter(answer => answer.idQuestion != currentQuestion.value.id));
  ...
  const finishQuestionnaire = () =>{
    getAnswersFromCurrentComponent();
    emit('evtPatientAnswers', toRaw(selectedAnswers.value) );
  };
